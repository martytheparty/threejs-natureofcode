...
let cellbody;
/*
 * when useControls true use controls for camera
 * when useControls false use last ball for camera
 */
let useControls = true;


let orbitClick = (caller) => {
  useControls = !useControls;
  let label = useControls ? 'orbit' : 'last ball'
  caller.innerHTML = label;
...
let zDisplacementFunction = (x, y) => {
  ...
  let absolutePosition = y + yDisplacement/2;
  let finalPos = y*(-1) + Simplex.noise(x, absolutePosition);
  return finalPos;
};
...
let generateSurfaceObjects = () => {
  ...
  surface.forEach(
    (position) => {
      ...
      options.rotation = {x: 0.1, y: 0, z: 0};
...
function uiSychronization() {
  ...
  if (world ) {
    ...
    fallingSpheres.forEach(
      (sphere, i) => {
        if (i === 0 && !useControls){
          camera.position.x = sphere.cannon.position.x;
          camera.position.y = sphere.cannon.position.y;
          camera.position.z = sphere.cannon.position.z;
        }
