...
function addTerrain() {
  ...
  for(let i = 0; i < spots; i++) {
    ...
    let mplier = Math.cos(i/10);
    if (i > 40) {
      mplier =Math.cos(i/10)*(spots - i)/200;
    }
    mplier = mplier.toFixed(2);
    console.log(mplier);
    let options = {};
    let zposition = spots - i*22;
    let height = (spots - i)*mplier + offset;
    if (i > 39) {
      height = height * .1;
    }
    options.position = {x: 0, y: 1*height, z: zposition};
    options.rotation = {x: 1*Math.PI/2, y: 0, z: 0};
    options.dimensions = {width: 240, height: height, depth: 12};
    options.scene = scene;
    options.world = world;
...
function addFallingSphere(intialPosition, initialDimensions, mass) {
  let maxCount = 100;
  let options = {};
  options.position = {x: 0,y: 730, z: -100};
...
document.onclick = () => {
   addFallingSphere({x: 0,y: 130, z: -100}, {radius: 32}, 1);
}
