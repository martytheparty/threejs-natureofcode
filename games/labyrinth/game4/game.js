( () => {
    ...
    const view = {
        createMainView: (mesh, sphereMesh, goalMesh, goal1Mesh, goal2Mesh) => {
            ...
            if (goalMesh) threeObject.scene.add(goalMesh);
            if (goal1Mesh) threeObject.scene.add(goal1Mesh);
            if (goal2Mesh) threeObject.scene.add(goal2Mesh);
            threeObject.renderer.setSize(mainElement.width, mainElement.height);
            return threeObject;
        },
        createFrontView: (mesh, sphereMesh, goalMesh, goal1Mesh, goal2Mesh) => {
        ...
        createLeftView: (mesh, sphereMesh, goalMesh, goal1Mesh, goal2Mesh) => {
        ...
        createBallView: (mesh, sphereMesh, goalMesh, goal1Mesh, goal2Mesh) => {
        ...
        createViews: (meshDescription, sphereMeshDescription, goalMeshDescription, goal1MeshDescription, goal2MeshDescription) => {
            ...
            goal1Meshes.push(view.getRenderingMesh(goal1MeshDescription));
            goal1Meshes.push(view.getRenderingMesh(goal1MeshDescription));
            goal1Meshes.push(view.getRenderingMesh(goal1MeshDescription));
            goal1Meshes.push(view.getRenderingMesh(goal1MeshDescription));

            goal2Meshes.push(view.getRenderingMesh(goal2MeshDescription));
            goal2Meshes.push(view.getRenderingMesh(goal2MeshDescription));
            goal2Meshes.push(view.getRenderingMesh(goal2MeshDescription));
            goal2Meshes.push(view.getRenderingMesh(goal2MeshDescription));

            const views = [];
            views.push(view.createMainView(plaformMeshes[0], sphereMeshes[0], goalMeshes[0], goal1Meshes[0], goal2Meshes[0]));
            views.push(view.createFrontView(plaformMeshes[1], sphereMeshes[1], goalMeshes[1], goal1Meshes[1], goal2Meshes[1]));
            views.push(view.createLeftView(plaformMeshes[2], sphereMeshes[2], goalMeshes[2], goal1Meshes[2], goal2Meshes[2]));
            views.push(view.createBallView(plaformMeshes[3], sphereMeshes[3], goalMeshes[3], goal1Meshes[3], goal2Meshes[3]));
            return views;
        }
    }
    const physics = {
    ...
    const CollisionDetection = {
        ...
        checkForCollision2: () => {
            const sphereBody = floor.cannonObjects[floor.sphereLookup.cannonIndex];
            const goalX = goal1.x - sphereBody.position.x;
            const goalY = goal1.y - sphereBody.position.y;
            const sphere = new SAT.Circle(new SAT.Vector(), sphereBody.boundingRadius);
            const goalSquare = new SAT.Polygon(new SAT.Vector(goalX, goalY), [
            new SAT.Vector(goal.depth, goal.width),
            new SAT.Vector(-1*goal1.depth, goal1.width),
            new SAT.Vector(-1*goal1.depth, -1*goal1.width),
            new SAT.Vector(goal1.depth, -1*goal1.width)
           ]);
        ...
        checkForCollision3: () => {
            const sphereBody = floor.cannonObjects[floor.sphereLookup.cannonIndex];
            const goalX = goal2.x - sphereBody.position.x;
            const goalY = goal2.y - sphereBody.position.y;
            const sphere = new SAT.Circle(new SAT.Vector(), sphereBody.boundingRadius);
            const goalSquare = new SAT.Polygon(new SAT.Vector(goalX, goalY), [
            new SAT.Vector(goal.depth, goal.width),
            new SAT.Vector(-1*goal2.depth, goal2.width),
            new SAT.Vector(-1*goal2.depth, -1*goal2.width),
            new SAT.Vector(goal2.depth, -1*goal2.width)
           ]);
    ...
    function setView() {
        ...
        const goal1Geometry = new THREE.BoxGeometry(goal1.width*2, goal1.height*2, goal1.depth*2);
        const goal2Geometry = new THREE.BoxGeometry(goal2.width*2, goal2.height*2, goal2.depth*2);
        const material = new THREE.MeshBasicMaterial({ color: 0xC19A6B });
        const goalMaterial = new THREE.MeshBasicMaterial({ color: 0x8b2222 });
        const goal1Material = new THREE.MeshBasicMaterial({ color: 0x8b2222 });
        const goal2Material = new THREE.MeshBasicMaterial({ color: 0x8b2222 });
        ...
        const goal1Description = { goal1Geometry, goalMaterial, x: goal1.x, y: goal1.y, z: goal1.z };
        const goal2Description = { goal2Geometry, goalMaterial, x: goal2.x, y: goal2.y, z: goal2.z };
        ...
        const goal1MeshDescription = { geometry: goalGeometry, material: goal1Material, x: goal1Description.x, y: goal1Description.y, z: goal1Description.z };
        const goal2MeshDescription = { geometry: goalGeometry, material: goal2Material, x: goal2Description.x, y: goal2Description.y, z: goal2Description.z };
        ...
        floor.layers = view.createViews(meshDescription, sphereMeshDescription, goalMeshDescription, goal1MeshDescription, goal2MeshDescription);
    }
    ...
    function animate() {
        ...
        if (!collision1) collision1 = CollisionDetection.checkForCollision1();
        if (!collision2) collision2 = CollisionDetection.checkForCollision2();
        if (!collision3) collision3 = CollisionDetection.checkForCollision3();
        if (!win && collision1) {
            goalMeshes[0].material.color.r = 0;
            goalMeshes[0].material.color.b = 1;
        }

        if (!win && collision2) {
            goal1Meshes[1].material.color.r = 0;
            goal1Meshes[1].material.color.b = 1;
        }

        if (!win && collision3) {
            goal2Meshes[0].material.color.r = 0;
            goal2Meshes[0].material.color.b = 1;
        }
